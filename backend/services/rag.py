...
class RAGService:
    def __init__(self, embedding_provider: str = "gemini"):
        ...
        """
        self.client = QdrantClient("https://d6547155-728d-481c-b616-df5e5a8cde21.eu-west-2-0.aws.cloud.qdrant.io", api_key=os.getenv("QDRANT_APIKEY"))
        ...
        if self.embedding_provider == "gemini":
            ...
            raise ValueError(
                f"Невідомий провайдер ембедингів: {embedding_provider}. Використовуйте 'gemini' або 'huggingface'."
            )
        ...
        for chunk_batch in batch_generator(chunks, self.hf_batch_limit):
            try:
                ...
                if isinstance(embeddings, list) and len(embeddings) == len(chunk_batch):
                    ...
                    print(
                        f"Неочікуваний формат відповіді від Hugging Face: {type(embeddings)}"
                    )
                    continue

            except requests.exceptions.HTTPError as http_err:
                print(
                    f"HTTP помилка при отриманні ембедингів від Hugging Face: {http_err}"
                )
                print(f"Відповідь: {response.text}")
                continue
            except Exception as err:
    ...
    def _get_huggingface_embedding(self, text: str) -> list[float]:
        ...
        try:
            ...
            if isinstance(embedding, list) and len(embedding) > 0:
                ...
                print(
                    f"Неочікуваний формат відповіді від Hugging Face: {type(embedding)}"
                )
                return []
        except requests.exceptions.HTTPError as http_err:
            print(
                f"HTTP помилка при отриманні ембедингу запиту від Hugging Face: {http_err}"
            )
            print(f"Відповідь: {response.text}")
            return []
        except Exception as err:
        ...
        if hasattr(search_results, "points"):
            ...
            for point in search_results.points:
                if isinstance(point, tuple):
                    ...
                    results.append({"text": point[0].payload.get("text", "")})
                else:
                    # Якщо це об'єкт
                    results.append(
                        {
